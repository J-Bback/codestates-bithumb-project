...
const Home = (props: any) => {
	...
	const getDayToDayFluctate = (name: string, type: string) => {
		...
		const fluctatePrice =
			Math.round((currentPrice - yesterdayPrice) * 100) / 100;
		const fluctateRate =
			Math.round((fluctatePrice / yesterdayPrice) * 10000) / 100;
		if (type === "krw") {
					...
					{keys.map((name, i) => {
						...
						return (
							...
							>
								...
								>
									{`(${signPositiveNumber.signRatePositive(
										Number(dayToDayFluctateRate)
									)} %)`}
								</div>
								<AreaChart />
							</div>
	...
	const handleClickBody = (selectedCurrency: string) => {
		router.push(
			{ pathname: "/exchange", query: { selectedCurrency } },
			undefined,
			{ shallow: true }
		);
	};
		...
		return keys.map((name, i) => {
			...
			return (
				<tr
					...
					ref={tableRef}
					onClick={() => handleClickBody(name)}
				>
					<td style={{ width: "25px" }}>
						<div style={{ alignItems: "center" }}>
							{query.tab === "krw" && (
								<div
									className={styles.table_favorite}
									onClick={(e: any) => onAddFavorites(i, e)}
								>
					...
					<td className={styles.table_asset_row}>
						...
						<div
							style={{ fontSize: "12px", color: "#a4a4a4" }}
						>{`${name} / KRW`}</div>
						{/* </div> */}
					</td>
					<td
						style={{ width: "161px", padding: "0 13px", fontSize: "14px" }}
					>{`${costComma(currentPrice)} 원`}</td>
					<td
						style={
							Math.sign(Number(dayToDayFluctateRate)) === 1
								? {
					...
					>
						<span>{`${signPositiveNumber.signPricePositive(
							Number(dayToDayFluctate)
						)} 원 `}</span>
					...
					<td
						style={{ width: "192px", padding: "0 13px", fontSize: "14px" }}
					>{`${costComma(Math.round(Number(accTradeValue)))} 원`}</td>
					<td
						style={{ width: "65px", textAlign: "center", paddingLeft: "23px" }}
					>
						입금
					</td>
